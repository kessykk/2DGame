...
function startGame() {
    ...
    for(var i = 0; i< 4;i++){
      ...
      myObstacles[4] = new componentObstacle(41, 41,  myGameArea.canvas.width, myGameArea.canvas.height/2 , 5, 0, 1, true);
...
function newAttack(){
  ...
  if(obsCount > 0){
    myObstacles[4] = new componentObstacle(41, 41,  myGameArea.canvas.width, (myGameArea.canvas.height/newRandYPos +20), 5, 0, leftOrRight, true);
  }
}
...
*/
  ...
  if(this.attack == true){
    ...
    this.update = function(changeImg) {
      ...
      if(changeImg == true && this.image.src.indexOf("Defend") == -1 && this.image.src.indexOf("Red") == -1){
...
function createNewObstacle(count){
  ...
  myObstacles[count] = new componentObstacle(81, 81,  newRandXPos, newRandYPos,speedX,speedY, leftOrRight, false);
}
...
function updateGameArea() {
  ...
  myObstacles.forEach(function(myObstacleTemp, i , temp){
     ...
     if(obsCount> 0 && temp[i].height > 1 && ((temp[i].attack == false && ((temp[i].x > gamePieceXPos || temp[i].x +81 > gamePieceXPos)  && temp[i].x < gamePieceXPos+50))
      && ((temp[i].y > gamePieceYPos || temp[i].y+81 > gamePieceYPos ) && temp[i].y < gamePieceYPos+110)) ||

       ((temp[i].attack == true) && ((temp[i].x > gamePieceXPos || temp[i].x+ 31 > gamePieceXPos) && temp[i].x < gamePieceXPos+40)
    ...
    || myGamePieceLife <= 0){
      ...
      ctx.fillStyle = "red";
      ctx.textAlign = "center";
      ctx.fillText("Drag the kid to", myGameArea.canvas.width/2 , 100);
      ctx.fillText("desired position and restart", myGameArea.canvas.width/2 , 120);
